...
impl PushService {
    pub fn new(
        ...
        let client = reqwest::ClientBuilder::new()
            // personas e2 A1: pin the rustls backend explicitly. When this crate is built
            // in a workspace where another crate pulls reqwest's default features (which
            // enable native-tls / `default-tls`), reqwest is compiled with BOTH backends
            // and the builder default flips to native-tls — but this client is written for
            // rustls (`.tls_built_in_root_certs(false)` + a manual `add_root_certificate`).
            // Forcing rustls keeps the TLS path deterministic; it is a no-op when only
            // rustls is compiled in (the standalone case).
            .use_rustls_tls()
