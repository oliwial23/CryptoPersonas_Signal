...
impl ContentsStore for SqliteStore {
    ...
    async fn save_message(
        ...
        let Metadata {
            ...
            was_plaintext,
            server_timestamp: _,
            // Personas fork field — not persisted (no column for it); a message
            // reloaded from the store loses this and reports `None`, same as any
            // identified message. See content.rs's Metadata doc comment.
            sender_identity_key: _,
        } = metadata;
