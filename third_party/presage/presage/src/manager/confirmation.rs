...
impl<S: Store> Manager<S, Confirmation> {
    ...
    pub async fn confirm_verification_code(
        ...
        self.confirm_verification_code_impl(confirmation_code, None)
            .await
    }

    /// Personas fork (see `docs/SERVERLESS_SIGNAL_DESIGN.md` §6, `docs/FINDINGS.md`):
    /// identical to [`confirm_verification_code`](Self::confirm_verification_code),
    /// except the account's **ACI** identity keypair is `aci_identity_key_pair`
    /// instead of a freshly random one. The PNI identity keypair is unaffected —
    /// still freshly random — since PNI identity is unrelated to the sealed-sender
    /// certificate this exists to share.
    ///
    /// This is the actual fork point for the shared-phantom-identity scheme: every
    /// caller that derives the same `aci_identity_key_pair` (e.g. deterministically
    /// from a shared secret) ends up with an account whose real, individually-issued
    /// `SenderCertificate` embeds the *same* public identity key, while every other
    /// part of registration — phone number, uuid, device id, prekeys — stays
    /// entirely per-account and independent. Upstream presage has no seam for this:
    /// `confirm_verification_code` always calls `IdentityKeyPair::generate` inline
    /// with no way to override it, which is why this is a fork rather than a
    /// downstream wrapper.
    pub async fn confirm_verification_code_with_identity(
        self,
        confirmation_code: impl AsRef<str>,
        aci_identity_key_pair: IdentityKeyPair,
    ) -> Result<Manager<S, Registered>, Error<S::Error>> {
        self.confirm_verification_code_impl(confirmation_code, Some(aci_identity_key_pair))
            .await
    }

    async fn confirm_verification_code_impl(
        self,
        confirmation_code: impl AsRef<str>,
        aci_identity_key_pair: Option<IdentityKeyPair>,
    ) -> Result<Manager<S, Registered>, Error<S::Error>> {
        ...
        // Personas fork: the ACI identity keypair is caller-supplied when present
        // (the shared-phantom-identity scheme) instead of always freshly random.
        // PNI is unaffected — still freshly random, always — used in `register_account`
        // and below either way.
        self.store
            .set_aci_identity_key_pair(
                aci_identity_key_pair.unwrap_or_else(|| IdentityKeyPair::generate(&mut rng)),
            )
            .await?;
        self.store
