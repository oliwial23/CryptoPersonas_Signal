...
pub mod cli;
/// The signal-cli JSON-RPC surface.
///
/// Public because `transport-signal-cli` calls it in-process. The server used to reach
/// signal-cli by spawning *this crate's binary* at 12 call sites and parsing its stdout as
/// JSON — a subprocess round-trip to a library that was already in the workspace.
...
/// Public function that replaces `main()`, allowing external callers to invoke the CLI logic.
...
// (keep the rest of the code the same, no changes needed to other functions)
...
async fn handle_command(
    ...
    match cli.command {
        ...
        // CliCommands::JoinGroup { uri } => {
        //     client::join();
        //     client.join_group(cli.account, uri).await
        // }
