...
/// Named concrete type rather than `impl SubscriptionClientT`, so a long-lived caller can
/// hold the client in a struct field. `transport-signal-cli` keeps one connection open for
/// the life of the process instead of reconnecting per message.
pub async fn connect_tcp(tcp: impl ToSocketAddrs) -> Result<Client, std::io::Error> {
    let (sender, receiver) = super::transports::tcp::connect(tcp).await?;

    Ok(ClientBuilder::default().build_with_tokio(sender, receiver))
}

pub async fn connect_unix(socket_path: impl AsRef<Path>) -> Result<Client, std::io::Error> {
